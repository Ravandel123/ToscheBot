...
function fighter(id: string, overrides: Partial<CombatStats> = {}): Fighter {
   return {
      id,
      stats: { name: id, maxHp: 20, attackBonus: 0, defenseBonus: 0, strengthBonus: 1, consitutionBonus: 1, ...overrides },
   };
}
...
describe('simulateFight', () => {
   ...
   it('lets a vastly stronger fighter win', () => {
      vi.spyOn(Math, 'random').mockReturnValue(0.5);
      const strong = fighter('strong', { maxHp: 200, attackBonus: 100, defenseBonus: 100, strengthBonus: 50, consitutionBonus: 50 });
      const weak = fighter('weak', { maxHp: 5, attackBonus: 0, defenseBonus: 0, strengthBonus: 0, consitutionBonus: 0 });
