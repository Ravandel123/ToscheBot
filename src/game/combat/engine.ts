 ...
 * how cleanly the attack landed, and the defender's constitution.
 */
export function simulateFight(a: Fighter, b: Fighter): FightResult {
   ...
   for (let round = 0; round < MAX_ROUNDS && hp[a.id] > 0 && hp[b.id] > 0; round++) {
      ...
      if (attackRoll >= defenseRoll) {
         ...
         damage = Math.max(1, attacker.stats.strengthBonus + randomInt(1, 4) + cleanliness - defender.stats.constitutionBonus);
